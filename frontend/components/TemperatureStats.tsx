...
export default function TemperatureStats({
  ...
  const combinedHistory = useMemo(() => {
    ...
    const livePoint = {
      time: "Direct",
      value: avgTemp || (initialHistory.length > 23 ? initialHistory[23].temperature : 0),
      isLive: true
    };
  ...
  const criticalZones = districts.filter((d) => getTemperatureLevel(d.avg_temperature) >= 2).length;
        ...
        {!loading && !error && districts.length > 0 && (
          <>
            <div className="flex items-center justify-between text-sm text-gray-800 mb-4">
              <span className="font-medium text-gray-500">{totalSensors} capteurs actifs</span>
              {criticalZones > 0 && (
                ...
                .map((d) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex items-center justify-between px-4 py-3 rounded-full border transition-all hover:shadow-md"
                    >
